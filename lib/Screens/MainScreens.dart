...
class MainScreens extends StatefulWidget {
 final String idUser;
 final String nama;
 const MainScreens({Key? key, required this.nama, required this.idUser}) : super(key: key);
 @override
...
class _MainScreensState extends State<MainScreens> {
 ...
 Map<String, dynamic> body = {"isi_motivasi": isi, "iduser": widget.idUser}; // [Tambah IDUSER -> Widget.iduser]
 try {
 Response response = await dio.post(
 "$baseurl/vigenesia/api/dev/POSTmotivasi/",
 data: body,
 options: Options(contentType: Headers.formUrlEncodedContentType)); // Formatnya Harus Form Data
...
.get('$baseurl/vigenesia/api/Get_motivasi?iduser=${widget.idUser}'); // NGambil by data
 print(" ${response.data}");  
...
Map<String, dynamic> data = {
 ...
 // <-- headers: {"Content-type": "application/json"}, --> ini gak perlu
 ...
 var response = await dio.get('$baseurl/vigenesia/api/Get_motivasi'); // Ngambil by ALL USER
 print(" ${response.data}");
 if (response.statusCode == 200) {
 var getUsersData = response.data as List;
 var listUsers =
...
CircularProgressIndicator();
 ...
 late String trigger = "trigger";
 late String triggeruser = "triggeruser";
 @override
